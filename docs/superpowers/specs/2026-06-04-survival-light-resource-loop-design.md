...
- Placement uses the selected hotbar stack.
...
- `add_item(ItemId item_id, uint16_t count)`
- `can_consume_selected(uint16_t count) const`
- `consume_selected(uint16_t count)`
...
returns false and leaves inventory unchanged.
...
- If the block has no drop, existing break behavior can still remove it when
  appropriate.
...
- Placement first checks the selected block item, then applies the world
  mutation, then consumes one item only if the mutation succeeded.
...
- Inventory rejects additions that cannot fully fit.
